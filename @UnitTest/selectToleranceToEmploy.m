...
function [toleranceEmployed, isCustom] = selectToleranceToEmploy(globalTolerance, customTolerances, fieldName)
    ...
    fieldPath = strsplit(fieldName, '.');
    drilledFieldName = '';
    for k = 1:numel(fieldPath)
        fieldPath{k} = strrep(fieldPath{k}, '(1)','');
        if (k == 1) 
            drilledFieldName = fieldPath{k};
        else
            drilledFieldName = [drilledFieldName '.' fieldPath{k}];
        end
    end
   % fieldPath
    
    if (isstruct(customTolerances))
       names = getFlattenedStructFieldNames(customTolerances,'full');
    else
...
function NAMES = getFlattenedStructFieldNames(S,varargin)
    %   getFlattenedStructFieldNames Get structure field names in recursive manner.
    %
    %   NAMES = getFlattenedStructFieldNames(S) returns a cell array of strings containing the
    ...
    %
    %   NAMES = getFlattenedStructFieldNames(S,DEPTH) is an optional field which allows the depth
    ...
    %   s.
    %
    %   NAMES = getFlattenedStructFieldNames(...,'full') returns the names of fields which are
    ...
    %
    %   NAMES = getFlattenedStructFieldNames(...,'prefix') returns the names of the fields
    %   prefixed by the name of the input structure.
    %
    %   NAMES = getFlattenedStructFieldNames(...,'struct') returns only the names of fields
        ...
        for i = 1:length(fndstruct)
            ...
            if depth == -1 || size(find(fndstruct{i} == '.'),2) <= depth-1
                ...
                elseif isempty(structi)
                    error ('AATJ:getFlattenedStructFieldNames,NotFindStructure'              ...
